...
Widget _buildSummaryCell(String text, Color color) {
return Expanded(
child: Center(
child: Text(
text,
...
final transactions = snapshot.data?.docs
    .where((doc) => (doc['customer']?['name'] as String? ?? '').toLowerCase().contains(_searchQuery))
    .toList() ??
[];

if (transactions.isEmpty) {
return const Center(child: Text('No transactions found', style: TextStyle(color: _textColor)));
}
...
receivedAmount += invoice.givenAmount;
...
mainAxisAlignment: MainAxisAlignment.spaceBetween,
children: [
_buildSummaryCell('Total: ${numberFormat.format(totalAmount)}/-', Colors.blue),
_buildSummaryCell('Received: ${numberFormat.format(receivedAmount)}/-', Colors.green),
_buildSummaryCell('Pending: ${numberFormat.format(pendingAmount)}/-', Colors.red),
],
),
),
],
...
if (transactions.isEmpty) {
return const Center(child: Text('No pending transactions found', style: TextStyle(color: _textColor)));
}
...
icon: const Icon(Icons.clear, color: _secondaryTextColor, size: 18),
onPressed: () {
_searchController.clear();
setState(() => _searchQuery = '');
},
),
prefixIcon: const Icon(Icons.search, color: _secondaryTextColor, size: 18),
),
...
boxShadow: _isHovered ? [] : [BoxShadow(color: Colors.black.withOpacity(0.05), blurRadius: 6, offset: const Offset(0, 3))],
),
child: Center(
child: Text(
widget.label,
...
width: width,
child: Center(child: Text(text, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w600, fontSize: 13), overflow: TextOverflow.ellipsis)),
);
}
...
children: [
if (onView != null)
IconButton(icon: const Icon(Icons.remove_red_eye, color: Colors.blue, size: 16), onPressed: () => onView!(transactionDoc), tooltip: 'View Transaction'),
if (onEdit != null)
IconButton(icon: const Icon(Icons.edit, color: _primaryColor, size: 16), onPressed: () => onEdit!(transactionDoc), tooltip: 'Edit Transaction'),
if (onPrint != null)
IconButton(icon: const Icon(Icons.print, color: Colors.green, size: 16), onPressed: () => onPrint!(transactionDoc), tooltip: 'Print Invoice'),
if (onComplete != null)
IconButton(
icon: const Icon(Icons.check_circle, color: Colors.purple, size: 16),
